...
import { ESCALATION_TARGETS, approveEscalation, escalationGuidance, escalationHintMarker, normalizeEscalationArgs, sandboxDenialMarker } from '@deepseek-ai/dsh-sandbox'
...
export class FsSandboxController {
  ...
  /** The composition's sandbox mode, or undefined when the mounted backend does not confine. */
  ...
  /**
   * The mode-aware escalation paragraph for a mutating tool's description, or
   * `''` when the mounted backend does not confine — the fields are then
   * unadvertised, and the paragraph would teach a lever that does not exist.
   * @returns the paragraph to append to the tool description.
   */
  guidance(): string {
    return this.defaultMode === undefined ? '' : escalationGuidance(this.defaultMode, 'file operation')
  }
   ...
   * always carried as the workspace root. Normalizes the escalation argument
   * pair first: blank fields are omitted, a malformed pair is refused.
  ...
  async resolvePolicy(toolName: string, args: FsEscalationArgs, exec: ToolExecution): Promise<SandboxExecutionPolicy | undefined> {
    // Blank escalation fields count as omitted, so an ordinary mutation carrying
    // two empty strings asks nobody and retries nothing.
